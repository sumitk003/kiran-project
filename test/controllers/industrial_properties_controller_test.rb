...
class IndustrialPropertiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should get industrial_property index' do
    get industrial_properties_path
  ...
  test 'should get new industrial_property' do
    get new_industrial_property_path
    assert_redirected_to new_agent_session_path

    sign_in @agent
    get new_industrial_property_path
    assert_response :success
    assert_select 'h1', 'New Industrial property'
    sign_out :agent
  end

  test 'should create industrial_property' do
    post industrial_properties_path, params: new_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    assert_difference('IndustrialProperty.count') do
      post industrial_properties_path, params: new_property_params
    end
    created_property = IndustrialProperty.unscoped.last
    new_property_params[:industrial_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal new_property_params[:industrial_property][key], created_property.public_send(key.to_s).to_plain_text
      else
        assert_equal new_property_params[:industrial_property][key], created_property.public_send(key.to_s)
      end
    end
    assert_redirected_to industrial_property_url(created_property)
    follow_redirect!
  ...
  test 'should show property' do
    get industrial_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should get edit' do
    get edit_industrial_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should update property' do
    patch industrial_property_url(@property), params: update_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    patch industrial_property_url(@property), params: update_property_params
    assert_redirected_to industrial_property_url(@property)
    updated_property = Property.find(@property.id)
    update_property_params[:industrial_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal update_property_params[:industrial_property][key], updated_property.public_send(key.to_s).to_plain_text
      else
        assert_equal update_property_params[:industrial_property][key], updated_property.public_send(key.to_s)
      end
    end
    follow_redirect!
