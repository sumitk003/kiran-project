...
class ResidentialPropertiesControllerTest < ActionDispatch::IntegrationTest
  include Devise::Test::IntegrationHelpers

  setup do
    @property = properties(:residential_property)
    @agent = agents(:bill)
  end

  test 'should get residential_property index' do
    get residential_properties_path
  ...
  test 'should get new residential_property' do
    get new_residential_property_path
    assert_redirected_to new_agent_session_path

    sign_in @agent
    get new_residential_property_path
    assert_response :success
    assert_select 'h1', 'New Residential property'
    sign_out :agent
  end

  test 'should create residential_property' do
    post residential_properties_path, params: new_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    assert_difference('ResidentialProperty.count') do
      post residential_properties_path, params: new_property_params
    end
    created_property = ResidentialProperty.unscoped.last
    new_property_params[:residential_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal new_property_params[:residential_property][key], created_property.public_send(key.to_s).to_plain_text
      else
        assert_equal new_property_params[:residential_property][key], created_property.public_send(key.to_s)
      end
    end
    assert_redirected_to residential_property_url(created_property)
    follow_redirect!
  ...
  test 'should show property' do
    get residential_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should get edit' do
    get edit_residential_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should update property' do
    patch residential_property_url(@property), params: update_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    patch residential_property_url(@property), params: update_property_params
    assert_redirected_to residential_property_url(@property)
    updated_property = Property.find(@property.id)
    update_property_params[:residential_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal update_property_params[:residential_property][key], updated_property.public_send(key.to_s).to_plain_text
      else
        assert_equal update_property_params[:residential_property][key], updated_property.public_send(key.to_s)
      end
    end
    follow_redirect!
