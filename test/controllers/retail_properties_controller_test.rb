...
class RetailPropertiesControllerTest < ActionDispatch::IntegrationTest
  include Devise::Test::IntegrationHelpers

  setup do
    @property = properties(:retail_property)
    @agent = agents(:bill)
  end

  test 'should get retail_property index' do
    get retail_properties_path
  ...
  test 'should get new retail_property' do
    get new_retail_property_path
    assert_redirected_to new_agent_session_path

    sign_in @agent
    get new_retail_property_path
    assert_response :success
    assert_select 'h1', 'New Retail property'
    sign_out :agent
  end

  test 'should create retail_property' do
    post retail_properties_path, params: new_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    assert_difference('RetailProperty.count') do
      post retail_properties_path, params: new_property_params
    end
    created_property = RetailProperty.unscoped.last
    new_property_params[:retail_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal new_property_params[:retail_property][key], created_property.public_send(key.to_s).to_plain_text
      else
        assert_equal new_property_params[:retail_property][key], created_property.public_send(key.to_s)
      end
    end
    assert_redirected_to retail_property_url(created_property)
    follow_redirect!
  ...
  test 'should show property' do
    get retail_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should get edit' do
    get edit_retail_property_url(@property)
    assert_redirected_to new_agent_session_path
  ...
  test 'should update property' do
    patch retail_property_url(@property), params: update_property_params
    assert_redirected_to new_agent_session_path

    sign_in @agent
    patch retail_property_url(@property), params: update_property_params
    assert_redirected_to retail_property_url(@property)
    updated_property = Property.find(@property.id)
    update_property_params[:retail_property].keys.each do |key|
      if rich_text_attributes.include?(key)
        assert_equal update_property_params[:retail_property][key], updated_property.public_send(key.to_s).to_plain_text
      else
        assert_equal update_property_params[:retail_property][key], updated_property.public_send(key.to_s)
      end
    end
    follow_redirect!
